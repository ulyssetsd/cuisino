...
class RecipeRepository {
    ...
    async loadFromImages() {
        ...
        return pairs.map((pair, index) => 
            Recipe.fromImagePaths(String(index + 1).padStart(3, '0'), pair.recto, pair.verso)
        );
    ...
    async loadExistingRecipes() {
        ...
        Logger.info(`Loaded ${recipes.length} existing recipes from consolidated file`);
        return recipes;
    ...
    async saveAllRecipes(recipes, stats = {}) {
        ...
        Logger.success(`Saved ${recipes.length} recipes to consolidated file: ${filePath}`);
        return filePath;
    }
