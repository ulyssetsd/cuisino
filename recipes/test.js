...
async function testRecipeEntity() {
    ...
    console.log(`✓ Recipe validation: ${validation.valid ? 'valid' : 'invalid'}`);
...
async function testRecipeRepository() {
    ...
    try {
        ...
        console.log(`⚠ Could not test with existing recipes: ${error.message}`);
        return [];
    }
}
...
async function testImagePairing() {
    ...
    const mockImages = ['image_001.jpg', 'image_002.jpg', 'image_003.jpg', 'image_004.jpg'];
    const pairs = repository.groupImagePairs(mockImages, '/test/dir');
    
    console.log(`✓ Created ${pairs.length} image pairs from ${mockImages.length} images`);
...
async function runRecipesTests() {
    try {
        ...
        console.log(`Repository operations: ${existingRecipes ? 'PASS' : 'FAIL'}`);
...
module.exports = { runRecipesTests, testRecipeEntity, testRecipeRepository, testImagePairing };